...
output: pdf_document
params:
    path_to_github: "path_to_the_covid-19_github"
    path_to_data: "path_to_the_upstream_df"
    scorecard_files: "path_to_scorecards_separated_by_semicolons"
    forecaster_names: "names_separated_by_semicolons"
    locations_considered: "ALL_or_codes_separated_by_semicolons"
...
forecaster_names <- str_split(params$forecaster_names, ";")[[1]]
...
candidate_common_locations = intersect(sc_locations, df$location)
common_locations =
  if (params[["locations_considered"]] == "ALL") {
    candidate_common_locations
  } else {
    intersect(candidate_common_locations, strsplit(params[["locations_considered"]],";")[[1L]])
  }
num_locations = length(common_locations)
if (num_locations == 0)
  stop("There are no common locations under consideration at which all forecasters are predicting.")
...
if (!(response %in% df$variable_name)) {
  ...
  if (response ==  old_name & new_name %in% df$variable_name) {
    ...
    stop(sprintf("response from score cards is %s, which is not found in df.",
                 response),
         " Here are the variables in df: ",
         paste(unique(df$variable_name), collapse = ", "))
  }
}
```
...
score_card_diff %>%
  ...
  mutate(population_group = (cut(population,
                                 breaks = unique(quantile(population, 
...
for(i in 1:length(time_series_plots)){
  tryCatch(print(time_series_plots[[i]]), error=function(e) {print(sprintf("No locations considered in location group %d",i))})
  cat("\n")
}
