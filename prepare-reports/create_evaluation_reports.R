...
data_paths <- paste0(
  ...
  c(
    ...
    "upstream_df_state_2020-08-02_final_trimmed.Rdata",
...
scorecard_meta <- scorecard_meta %>% 
  mutate(forecaster = ordered(forecaster, 
                              levels = c("cdf_ens_v0_selectHub6",
                                         "COVIDhub-ensemble",
                                         "ensemble3_cdc_impute_calike",
...
levels(scorecard_meta$forecaster) <- dplyr::recode(levels(scorecard_meta$forecaster),
                                                   "hayman_ensemble_v1_selectHub2" = "hayV1S2",
                                                   "COVIDhub-ensemble" = "CHens",
                                                   "COVIDhub-base" = "CHbase",
                                                   "hayman_ensemble_v1_selectHub4" = "hayV1S4",
                                                   "hayman_ensemble_v0_selectHub2" = "hayV0S2",
                                                   "hayman_ensemble_v0_selectHub4" = "hayV0S4",
                                                   "hayman_ensemble_v1_selectHub7" = "hayV1S7",
                                                   "ensemble3_cdc_impute" = "qg3cdc_imp",
                                                   "ensemble3_cdc_impute_calike" = "qg3cdc_imp_CAlk",
                                                   "cdf_ens_v0_selectHub6" = "cdfV0S6"
                                                   )
