...
output_path_1  <- "../../covid-19-iif-blog-post-data/smallcards/verification_ensembles"
...
for (ahead in 1L) {
  for (geo_type in names(responses)) {
    ...
    ensemble_forecasters = lemur::get_dev_forecasters(response=response, incidence_period=incidence_period, ahead=ahead,
                                                      ...
                                                      debug_weights_folder_suffix="_verificationrun")
    ensemble_forecasters <- ensemble_forecasters[grepl("verification", names(ensemble_forecasters))]
    forecast_dates = lubridate::ymd("2020-10-05")
