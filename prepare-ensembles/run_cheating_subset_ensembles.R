...
output_path_1  <- "../../covid-19-iif-blog-post-data/smallcards/cheating_subset_ensembles"

incidence_period <- "epiweek"
n_locations <- 200

forecaster_full_mmweights = readRDS("../../covid-19-iif-blog-post-data/debug_quantgen_weights/forecaster_full_mmweights.RDS")
stopifnot(!is.unsorted(rev(forecaster_full_mmweights[["df"]][["mmweight"]])))

for (subset_size in c(4L,8L,16L)) {
  cat(sprintf("SUBSET SIZE %d\n", subset_size))

  subset_forecasters = forecaster_full_mmweights[["df"]][["forecaster"]][seq_len(subset_size)]
  for (ahead in 1:4) {
