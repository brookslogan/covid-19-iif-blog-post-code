...
#'
#' @importFrom fs path
#' @importFrom dplyr bind_rows count filter mutate pull
...
get_quantile_forecasts <- function(train_dates,
  ...
  for (train_date in as.character(train_dates)) {
    ...
    for (forecaster_i in seq_along(forecasters)) {
      ...
      file_basenames = if (from_scorecard) "scorecard.RDS" else c("out.RDS","scorecard.RDS")
      try({
        ...
        possible_scorecard_filepaths =
          ...
          mutate(filepath = path(components_dirpath,
                                 ...
                                 file_basename)) %>%
          pull(filepath)
        possible_scorecard_exists = file.exists(possible_scorecard_filepaths)
        if (!any(possible_scorecard_exists)) {
          stop (sprintf("Could not find scorecard at any of the following paths:\n%s",
                        paste(collapse="\n", capture.output(print(possible_scorecard_filepaths)))))
        } else {
            scorecard_filepath = possible_scorecard_filepaths[[which(possible_scorecard_exists)[[1L]]]]
            s = readRDS(scorecard_filepath)
            scorecard_list[[forecaster]] <-
    ...
    #if (length(scorecard_list) != length(forecasters) || anyNA(scorecard_list)) {
    #  next
    #}
    if (length(scorecard_list) == 0) {
      next
    }
    scorecards_one_day = bind_rows(scorecard_list[!is.na(scorecard_list)], .id="forecaster")
    ## For now, we throw away any location that isn't predicted by every forecaster
    ## good_locations = (scorecards_one_day %>% count(location) %>% filter(n==length(forecasters)))[["location"]]
    ## scorecards_one_day = scorecards_one_day %>% filter(location %in% good_locations)
    encountered_locations = unique(scorecards_one_day[["location"]])
    all_locations =
    ...
    for (i in seq_len(dim(scorecards_one_day)[[1L]])) {
      line = scorecards_one_day[i,]
    ...
    if (from_scorecard) {
      actual_values = c(actual_values, purrr::map_dbl(all_locations, function(loc) { scorecards_one_day[which(scorecards_one_day$location == loc)[1],][["actual"]]}))
    }
  }
...
quantgen_ensemble_forecaster_v0 <- function(response, incidence_period, ahead,
    ...
    my_forecaster = function(df, forecast_date) {
        ...
        component_forecasts = get_quantile_forecasts(as.character(forecast_date),forecasters, ahead=ahead, geo_type=geo_type, incidence_period=incidence_period,
                                                     n_locations=n_locations,response=response,repo_root_dirpath=repo_root_dirpath,from_scorecard=cheating_fit_on_test_date_instead)
        ...
        success = tryCatch(expr={
            qf =
              if (cheating_fit_on_test_date_instead) {
                ...
                get_quantile_forecasts(train_dates, forecasters, ahead=ahead, geo_type=geo_type, incidence_period=incidence_period,
